...
if(!(global.paused))
{
	...
	if(abs(xvelocity)>10){xvelocity = xvelocity * 0.7}
	if(abs(yvelocity)>10){yvelocity = yvelocity * 0.7}
	...
	*/
	...
	if(((KeyUp!=0) or (KeyDown!=0) or (KeyLeft!=0) or (KeyRight!=0)))
	{
		sprite_index = sprBoatSplash2;
		image_index += 0.05;
	}
	
	if(!(((KeyUp!=0) or (KeyDown!=0) or (KeyLeft!=0) or (KeyRight!=0))))
	{
		sprite_index = sprBoat2;
		image_index = 0;
	}/**/
}